...
> **★ 规范承载定位**：信息收集规范（弹窗选项、字段映射、占位符检测、输出 JSON 消费映射）**全部由本文档承载**。子技能（standalone 模式）和父技能（proxy 模式）**仅代理引用本文档**，不保存冗余规范描述。
>
> 本文档同时也是 `collect-user-info.py` 的静态参考镜像，供 LLM 在执行信息收集前预读理解。运行时由 `collect-user-info.py` 输出 JSON 规范，本文档不参与运行时执行。
>
> **代理关系**：子技能本身也需要信息收集前置（standalone 模式 6 项，不含调度间隔）；父技能 proxy 模式**代理子技能的这部分工作**，并**额外增加调度间隔收集**（父技能专有字段），共 7 项。
